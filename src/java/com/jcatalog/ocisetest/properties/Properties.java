...
public class Properties extends LinkedHashMap {
    ...
    /**
     * Calls the <tt>LinkedHashMap</tt> method <code>put</code>. Provided for parallelism
     * with the <tt>getProperty</tt> method. Enforces use of strings for property keys
     * and values. The value returned is the result of the <tt>LinkedHashMap</tt> call
     * to <code>put</code>.
     *
     ...
     * used for character and string literals (see <a
     ...
     * </li>
     * <li>
     * The character sequence <code>\b</code> does <i>not</i> represent a backspace
     * character.
     * </li>
     * <li>
     * The method does not treat a backslash character, <code>\</code>, before a
     * non-valid escape character as an error; the backslash is silently dropped.  For
     * example, in a Java string the sequence <code>"\z"</code> would cause a compile
     * time error.  In contrast, this method silently drops the backslash.  Therefore,
     * this method treats the two character sequence <code>"\b"</code> as equivalent to
     * the single character <code>'b'</code>.
     * </li>
     * <li>
     ...
     * may be either a blank line, a comment line, or hold some part of a key-element
     * pair.  The logical line holding all the data for a key-element pair may be spread
     * out across several adjacent natural lines by escaping the line terminator
     * sequence with a backslash character, <code>\</code>.  Note that a comment line
     * cannot be extended in this manner; every natural line that is a comment must have
     * its own comment indicator, as described below.  If a logical line is continued
     * over several natural lines, the continuation lines receive further processing,
     ...
     * its first non-white space character; comment lines are also ignored and do not
     * encode key-element information.  In addition to line terminators, this method
     * considers the characters space (<code>' '</code>, <code>'&#92;u0020'</code>), tab
     ...
     * at the start the following line have no affect on the key or element values. The
     * remainder of the discussion of key and element parsing will assume all the
     * characters constituting the key and element appear on a single natural line after
     * line continuation characters have been removed.  Note that it is <i>not</i>
     * sufficient to only examine the character preceding a line terminator sequence to
     * to see if the line terminator is escaped; there must be an odd number of
     * contiguous backslashes for the line terminator to be escaped.  Since the input is
     * processed from left to right, a non-zero even number of 2<i>n</i> contiguous
     ...
     * <code>'='</code>, <code>':'</code>, or white space character other than a line
     ...
     * after the key is <code>'='</code> or <code>':'</code>, then it is ignored and any
     * white space characters after it are also skipped.  All remaining characters on
     * the line become part of the associated element string; if there are no remaining
     * characters, the element is the empty string <code>&quot;&quot;</code>.  Once the
     ...
     * after each comma in the final result; the <code>\</code>, line terminator, and
     ...
     *
     * @deprecated This method does not throw an IOException if an I/O error occurs while
     *             saving the property list.  As of the Java 2 platform v1.2, the
     ...
     * then the associated element string. Each character of the key and element strings
     * is examined to see whether it should be rendered as an escape sequence. The ASCII
     * characters <code>\</code>, tab, form feed, newline, and carriage return are
     * written as <code>\\</code>, <code>\t</code>, <code>\f</code><code>\n</code>, and
     * <code>\r</code>, respectively. Characters less than <code>&#92;u0020</code> and
     ...
     * For the key, all space characters are written with a preceding <code>\</code>
     * character.  For the element, leading space characters, but not embedded or
     * trailing space characters, are written with a preceding <code>\</code> character.
     * The key and element characters <code>#</code>, <code>!</code>, <code>=</code>,
     ...
     * @param header a description of the property list.
     *
     * @exception IOException if writing this property list to the specified output
     ...
     * is not found in this property list, the default property list, and its defaults,
     * recursively, are then checked. The method returns <code>null</code> if the
     ...
     * Searches for the property with the specified key in this property list. If the key
     * is not found in this property list, the default property list, and its defaults,
     * recursively, are then checked. The method returns the default value argument if
     ...
     *
     * @return an enumeration of all the keys in this property list, including the keys
